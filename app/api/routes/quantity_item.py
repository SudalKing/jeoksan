...
def search_quantity_item(
    ...
    result = quantity_item_service.search(db, work_type_code, level_codes, keyword, has_price, page, size)
    return success(result.model_dump(mode="json"))


@router.get("/quantity-item/{item_code}")
def get_quantity_item(item_code: str, db: Session = Depends(get_db)):
    result = quantity_item_service.get_one(db, item_code)
    if result is None:
